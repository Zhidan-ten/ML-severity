function feats = Tfeats2D(image)%二维纹理特征(147)
% clear
addpath('F:\Program Files\MATLAB\workspace\GLRL\');
warning('off','all');
% image = importdata('2.png');
roi = image;


area = sum(sum(roi~=0));%面积，直接简单求像素点
...
% 1~15
feats = [feats;area;energy;entropy(Vals);kurtosis(Vals);max(Vals);mean(Vals);...
